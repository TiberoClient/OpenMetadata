...
TEST_TIBERO_GET_STORED_PACKAGES = textwrap.dedent(
    """
SELECT
    OWNER,
    NAME,
...
TIBERO_ALL_TABLE_COMMENTS = textwrap.dedent(
    """
SELECT
...
TIBERO_ALL_VIEW_DEFINITIONS = textwrap.dedent(
    """
SELECT
...
TIBERO_GET_STORED_PROCEDURES = textwrap.dedent(
    """
SELECT
    OWNER,
    NAME,
...
TIBERO_GET_TABLE_NAMES = textwrap.dedent(
    """
...
TIBERO_IDENTITY_TYPE = textwrap.dedent(
    """\
col.default_on_null,
(
...
TIBERO_GET_STORED_PACKAGES = textwrap.dedent(
    """
SELECT
    OWNER,
    NAME,
...
TIBERO_GET_STORED_PROCEDURE_QUERIES = textwrap.dedent(
    """
WITH SP_HISTORY AS (SELECT
	s.sql_text AS query_text,
    TO_TIMESTAMP(s.FIRST_LOAD_TIME, 'YYYY-MM-DD HH24:MI:SS') AS start_time,
    TO_TIMESTAMP(s.LAST_ACTIVE_TIME, 'YYYY-MM-DD HH24:MI:SS') + NUMTODSINTERVAL(s.ELAPSED_TIME / 1000, 'SECOND') AS end_time,
 ...
 Q_HISTORY AS (SELECT
      s.sql_text AS query_text,
      CASE 
      	WHEN UPPER(s.SQL_TEXT) LIKE 'INSERT%' THEN 'INSERT'
      	WHEN UPPER(s.SQL_TEXT) LIKE 'SELECT%' THEN 'SELECT'
      	ELSE 'OTHER'
    	END AS QUERY_TYPE,
      TO_TIMESTAMP(s.FIRST_LOAD_TIME, 'YYYY-MM-DD HH24:MI:SS') AS start_time,
      TO_TIMESTAMP(s.LAST_ACTIVE_TIME, 'YYYY-MM-DD HH24:MI:SS') 
      + NUMTODSINTERVAL(s.ELAPSED_TIME / 1000, 'SECOND') AS end_time,
      u.username AS user_name,
...
TIBERO_GET_COLUMNS = textwrap.dedent(
    """
        SELECT
            col.column_name,
            col.data_type,
...
TIBERO_ALL_CONSTRAINTS = textwrap.dedent(
    """
        SELECT
            ac.constraint_name,
            ac.constraint_type,
...
TIBERO_QUERY_HISTORY_STATEMENT = textwrap.dedent(
    """
SELECT 
  NULL AS user_name,
  NULL AS database_name,
...
FROM gv$sql
WHERE SQL_TEXT NOT LIKE '/* {{"app": "OpenMetadata", %%}} */%%'
    AND SQL_TEXT NOT LIKE '/* {{"app": "dbt", %%}} */%%' {filters}
